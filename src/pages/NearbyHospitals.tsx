...
export default function NearbyHospitals() {
  ...
  useEffect(() => {
    ...
    getHospitals().then((allHosp) => {
      const currentCity = hospital.city.trim().toLowerCase();
      const otherHospitals = allHosp
        .filter((h) => h.id !== hospital.id && h.city.trim().toLowerCase() === currentCity)
        .map((h) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 transition-colors duration-200">
      {/* Header */}
      <div className="mb-8">
        <span className="text-xs font-bold uppercase tracking-wider text-teal-700 dark:text-teal-300 bg-teal-50 dark:bg-teal-950/60 px-3 py-1 rounded-full border border-teal-200 dark:border-teal-800">
        ...
        <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
          When your local donor pool is insufficient, request instant cold-chain transfers from nearby accredited medical centers in {hospital.city}.
        </p>
      </div>
      ...
      {successNotice && (
        <motion.div
          ...
          className="mb-6 p-4 bg-teal-50 dark:bg-teal-950/60 text-teal-900 dark:text-teal-200 border border-teal-200 dark:border-teal-800 rounded-2xl flex items-center gap-3 font-bold text-sm"
        >
      ...
      {/* Map View */}
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-md p-6 sm:p-7 mb-8">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-white text-sm sm:text-base">City Hospital Network Map — {hospital.city}</h3>
              <p className="text-xs text-slate-500 dark:text-slate-400">Live GPS proximity relative to {hospital.hospital_name}</p>
            </div>
          </div>
        </div>
      ...
      {/* Filter & Search Bar */}
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-sm p-4 sm:p-5 mb-8 flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Search className="w-5 h-5 text-slate-400" />
          <select
            value={filterType}
            onChange={(e) => setFilterType(e.target.value)}
            className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 dark:bg-slate-900 text-xs font-bold text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary-500/40"
          >
        ...
        <span className="text-xs font-bold text-slate-500 dark:text-slate-400">
          {filtered.length} Hospitals in {hospital.city} with Available Stock
        </span>
      </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {filtered.map((h, i) => (
          <motion.div
            ...
            className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-md p-6 flex flex-col justify-between hover:shadow-lg transition-all"
          >
            <div>
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 bg-teal-50 dark:bg-teal-950/60 text-teal-700 dark:text-teal-300 rounded-2xl flex items-center justify-center font-bold">
                    <Building2 className="w-6 h-6" />
                  </div>
                  <div>
                    <h4 className="font-black text-slate-900 dark:text-white text-base">{h.hospital_name}</h4>
              ...
              <div className="flex items-center gap-4 text-xs font-bold text-slate-600 dark:text-slate-300 mb-4 bg-slate-50 dark:bg-slate-900/60 p-2.5 rounded-xl border border-slate-100 dark:border-slate-700">
      ...
      {transfers.length > 0 && (
        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-md overflow-hidden">
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-700">
            {transfers.map((t) => (
              <div key={t.id} className="p-5 flex items-center justify-between hover:bg-slate-50/70 dark:hover:bg-slate-700/50 transition-colors">
                ...
                <span className="px-3 py-1 bg-teal-100 dark:bg-teal-950/60 text-teal-800 dark:text-teal-300 text-xs font-bold rounded-xl border border-teal-200 dark:border-teal-800">
                  {t.status}
                </span>
              </div>
            ))}
